...
router.get("/callback", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: "Authentication failed", profile });
  }
  try {
    const existingUser = await User.findOne({ spotifyId: profile.id });
    if (existingUser) {
      existingUser.refreshToken =
        tokenSet.refresh_token || existingUser.refreshToken;
      existingUser.accessToken = tokenSet.access_token;
      existingUser.accessTokenExpiresAt = new Date(
    ...
    res.status(500).json({ message: "Authentication failed", profile });
  }
});
